...
def handle_connection_failure(func):
    ...
    def _inner(*args, **kwargs):
        try:
            ...
            logger.debug("The cacheops cache is unreachable! Error: %s" % e)
            warnings.warn("The cacheops cache is unreachable! Error: %s" % e)

    return _inner

class SafeRedis(redis.StrictRedis):
    get = handle_connection_failure(redis.StrictRedis.get)
    set = handle_connection_failure(redis.StrictRedis.set)
